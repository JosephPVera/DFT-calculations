...
def load_yaml_file(yaml_file):
    ...
    return data
yaml_file = '../unitcell/unitcell.yaml'  
...
class EigenvalueMplPlotter(EigenvaluePlotter):
    def __init__(self,**kwargs): ###, figsize=(10, 6)
        super().__init__(**kwargs)
        self._mpl_defaults = kwargs.get("mpl_defaults", EigenvalueMplSettings())
        self.plt = plt

        if len(self._energies_and_occupations) == 2:
            self.fig, self.axs = plt.subplots(nrows=1, ncols=2, sharey='all') #, figsize=figsize
        else:
            self.fig, ax = plt.subplots(nrows=1, ncols=1, sharey='all') #, figsize=figsize
            self.axs = [ax]
    ...
    def _add_eigenvalues(self):
        handles = [] ###############################################
        labels = [] ###############################################
        legend_circle_size = self._mpl_defaults.circle_size * 3  # Adjust the multiplier as needed
 ...
 #                   color = "b" if occup > 0.9 else "r" if occup < 0.1 else "g"
 #                    ax.scatter([kpt_idx], energy, marker="o", color=color,
 #                             s=self._mpl_defaults.circle_size)
                    # Custom label assignment based on occupancy
                    if occup > 0.9:
                       color = "b"
                       label = "Occupied"
                    elif occup < 0.1:
                ...
                # Annotate the plot with the custom labels
                    ax.scatter([kpt_idx], energy, marker="o", color=color,
                               s=self._mpl_defaults.circle_size)
                    
                    if label not in labels:
                        handles.append(ax.scatter([], [], marker="o", color=color, s=legend_circle_size))
                        labels.append(label)           
                    ...
                    try:
                     ...
                     # Introduce the index for each state
...
#                                    fontsize=self._mpl_defaults.tick_label_size)
        ax.legend(handles, labels, loc='best', fontsize=self._mpl_defaults.tick_label_size) #########
    ...
    def _add_band_edges(self):
        for ax in self.axs:
#            ax.axhline(y=vbm, **self._mpl_defaults.svline) #########################################
#            ax.axhline(y=cbm, **self._mpl_defaults.svline) #########################################
            ax.axhspan(vbm, vbm - 2, color='lightblue', alpha=0.4) #######################
            ax.axhspan(cbm, cbm + 2, color='thistle', alpha=0.4) #########################
    ...
    def _set_y_range(self):
        self.plt.ylim(vbm - 2, cbm + 2) ##########################
#        if self._y_range:
#            for ax in self.axs:
#                ax.set_ylim(self._y_range)

    def _set_labels(self):
        self.fig.text(0.5, 0.01, "K-point coordinates", ha='center', #############################
                      size=self._mpl_defaults.label_font_size)
        self.axs[0].set_ylabel(f"Energy ({self._y_unit})",
                        size=self._mpl_defaults.label_font_size)

    def _set_title(self):
        for ax, spin in zip(self.axs, ["Spin up", "Spin down"]):
            ax.set_title(f"{spin}")
