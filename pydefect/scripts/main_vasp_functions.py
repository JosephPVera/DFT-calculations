...
# Inside the make_perfect_band_edge_state function
def make_perfect_band_edge_state(args):
    ...
    # Save the plot with the folder's name
    folder_name = args.dir.name
    plotter.plt.savefig(fname=args.dir / f"{folder_name}.png", dpi=150)  ###############################################
    plotter.plt.clf()


# Inside the make_band_edge_orb_infos_and_eigval_plot function
def make_band_edge_orb_infos_and_eigval_plot(args):
    ...
    def _inner(_dir: Path):
        ...
        plotter.plt.savefig(fname=_dir / f"{folder_name}.png", dpi=150)  ##############################################
        plotter.plt.clf()
